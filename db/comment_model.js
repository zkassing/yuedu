...
class CommentModel {
  ...
  static comments_select_limit_by_articleid (values) {
    const _sql = `SELECT id, user_id, (SELECT nikiname FROM t_users WHERE id = t_comments.user_id) as nikiname, (SELECT avatar FROM t_users WHERE id = t_comments.user_id) as avatar, article_id, content, DATE_FORMAT(created_time, '%Y/%m/%d %H:%i') as reply_time FROM t_comments WHERE article_id = ? LIMIT ?`
